...
const CanvasPage = forwardRef<CanvasPageHandle, CanvasPageProps>(({ color, thickness, mode, zoom, fontSize, fontFamily }, ref) => {
  ...
  const setImageRef = useCallback((node: Konva.Image | null) => {
    if (selectedId !== null && elements[selectedId]?.type === 'image') {
  ...
  const handleMouseDown = (e: any) => {
    ...
    switch (mode) {
      ...
      case 'rect':
        setCurrent({ type: 'rect', x: pos.x, y: pos.y, width: 0, height: 0, stroke: color, strokeWidth: thickness });
        break;
      case 'circle':
      case 'triangle':
        const dx = 0;
        const dy = 0;
        setCurrent({ type: mode, x: pos.x, y: pos.y, radius: 0, stroke: color, strokeWidth: thickness });
        break;
      case 'arrow':
        setCurrent({ type: 'arrow', points: [pos.x, pos.y, pos.x, pos.y], stroke: color, strokeWidth: thickness });
        break;
      case 'text': {
        if (!clickedOnEmpty) return;
        const newText = prompt('Enter text:') || '';
        setElements([...elements, { type: 'text', x: pos.x, y: pos.y, text: newText, fontSize, fontFamily, fill: color }]);
        setRedoStack([]);
        break;
      }
    }
  ...
  return (
    <div onDrop={handleDropImage} onDragOver={(e) => e.preventDefault()} style={{ width: '100%', height: '500px' }}>
      <Stage
        ref={stageRef}
        ...
        <Layer>
          {[...elements, current].filter(Boolean).map((el, i) => {
            ...
            switch (el.type) {
              ...
              case 'text':
                return <Text key={i} {...el} draggable stroke={isSelected ? 'blue' : undefined} strokeWidth={isSelected ? 0.5 : 0} onClick={() => setSelectedId(i)} onDblClick={() => setEditingId(i)} onDragEnd={(e) => {
                  const newElements = [...elements];
              ...
              case 'image':
                return (
                  <KonvaImage
                    ...
                    draggable
                    ref={isSelected ? setImageRef : null}
